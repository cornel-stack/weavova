...
`specs/T4-B3-warmth-sort/plan.md` (active slice: T4-B3 — Warmth sort: the inbox re-ordered by content-readiness, making the long-stubbed "Warmest — coming soon" sort control real. Q1:A all four owned signals (completeness + un-tapped + recency, consent as a GATE); Q2:A sort-order only, NO per-proof badge/number, honest copy on the control; Q3:A toggle default stays Newest (opt-in). Warmth = transparent read-time lexicographic order: granted ≻ non-granted → readiness points (words +2 / media +1 / un-tapped +2) → recency (capturedAt) → id. New pure src/lib/warmth.ts (warmthCompare/sortByWarmth, type-only ProofView import). The un-tapped signal's clip-status is NOT on ProofView (B4 headline-gap pattern) → additive getProofClipStatus read behind an OPT-IN-LAZY getInboxClipStatus server action that fires ONLY on toggle to Warmest (default Newest path byte-stable, extra read never fires by default). Consent reuses the effective state already on ProofView as the cold-gate; getProofs stays unfiltered (inbox shows all states; withdrawn ranks cold but visible). NO schema change (read-time, no stored warmth), NO new dep, NO new route. PLAN done — no ratification needed. Byte-stable: getProofs SHAPE/ProofView, ProofCard, proof·clip·showcase reads, generateClip, generateBatch, nav rail, default Newest order. The two in-scope edits are inbox-toolbar + inbox-client.)
<!-- SPECKIT END -->
