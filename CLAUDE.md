...
`specs/T4-B4-export/plan.md` (active slice: T4-B4 — Export: take a proof clip OUT as post-ready content. Q1:B single + bulk; Q2:A sample stays the openly-labeled placeholder (SAMPLE_CLIP_URL untouched); Q3:C copy-to-clipboard for single, download ONE JSON manifest for bulk. New additive consent-gated reads (getClipExport/getClipExports — the customer headline gap: quote/transcript absent from existing clip read shapes) + src/lib/export.ts (PostTextPackage, formatPostText, buildManifest) + exportClips Server Action (re-checks consent at read time, the B1 race) + a clip-detail copy island + a Library selection wrapper (B1 InboxClient pattern, byte-stable clip card). Honest T8 video seam: never a finished-clip file. NO schema change, NO new route/nav entry. PLAN done — one determination to ratify: the dependency question is ZERO new deps (native clipboard + native Blob, no zip — research.md §1); manifest format JSON recommended, non-blocking (§2). ProofCard/proof·clip·showcase read shapes/generateClip/generateBatch/LibraryClipCard/nav rail byte-stable).
<!-- SPECKIT END -->
