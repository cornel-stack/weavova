...
`specs/T7.1-scoped-consent-model/plan.md` (active slice: T7.1 — Scoped consent model; the foundation slice of T7 (Capture). SCHEMA + read-helper evolution, NO new UI (the capture form is T7.2). Evolves consent from binary granted/awaiting/revoked into a record that ALSO carries a `ConsentDisplay` payload on EACH version: `useScope[]` (organic|paid|showcase|embed), `nameDisplay` (full|first_initial|anonymous), `showFace`. P-VII mechanism UNCHANGED (withdrawal = new revoked version, read-time effective, retained; Leo M. behaves identically). STORAGE CRUX resolved: typed columns throughout — `useScope` as a `consent_scope[]` enum-array + GIN index so the T9 scope gate is ONE indexed containment predicate (`use_scope @> array['paid']`), never a jsonb scan; `nameDisplay` enum + `showFace` boolean (presentation-only); NO jsonb for ConsentDisplay (existing `consent.captureContext` untouched). Workspace display-default = additive `default_name_display`/`default_show_face` columns on the WORKSPACE row (Q1=A; brand_kit rejected — multi-row). Additive migration 0005: 2 enums + 3 consent cols (`use_scope` NOT NULL DEFAULT '{}', `name_display`/`show_face` nullable) + 2 workspace cols + GIN index + a hand-appended IDEMPOTENT backfill UPDATE (granted→full scope, display→full/true). Read path (ADDITIVE, byte-stable): new `latestConsentScope/NameDisplay/ShowFace` siblings (mirror latestConsentVersion); the FAILS-CLOSED scope gate in two forms — `effectiveConsentGrantsScope(col, scope): SQL` predicate (`...is true` coerces no-row→false) + `consentGrantsScope(ws, proof, scope): Promise<boolean>`; `getEffectiveConsentDisplay(ws, proof)` clean entry point (not called by any T7.1 surface); `getGrantedConsentId` return WIDENED additively to {consentId, useScope, nameDisplay, showFace} (.consentId byte-stable). ENFORCEMENT POINT = pure `resolveDisplay(wsDefault, override)` in src/lib/consent.ts — one-directional privacy (override may only raise privacy: full<first_initial<anonymous, showFace true<false; less-private clamped to default), + `DEFAULT_USE_SCOPE=['organic']` (least-privilege for NEW captures), + `BUILTIN_DISPLAY_DEFAULT={first_initial,true}` fallback when ws cols null. Why FR-010 consumers (inbox/library/dashboard/export/warmth/showcase/clip-detail/proof-detail/T5 ledger/generate gates) are byte-stable BY CONSTRUCTION: they all gate on `effectiveConsentGranted` = state='granted' ONLY — they never read scope, so the new columns are invisible to them; the scope gate is net-new (T9 consumes). Backfill honest (P-XIV) = restores the full-trust behaviour the fixtures already had, distinct from the privacy-forward first_initial workspace default for new captures. No test framework (P-III) — verify via typecheck+lint+build + reseed read-path checks (quickstart). No new dependency. Constitution: P-VII unchanged (Leo M. verified), P-XIV honest backfill, P-V seam byte-stable, P-VIII/P-XIII/P-XV/P-XVI N/A (non-UI/non-render). PLAN + research/data-model/contracts/quickstart done; NO open clarifications (Q1 resolved = workspace row). Ready for /speckit.tasks.)
<!-- SPECKIT END -->
