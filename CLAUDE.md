...
`specs/T5-consent/plan.md` (active slice: T5-Consent — Consent surface: a real management view over the consent model that already gates every surface, plus a live record-withdrawal action. The first T5 slice (T4 complete). Q1:A cascade-preview confirm; Q2:A light-include made-under provenance; Q3:A full retained version timeline. Replaces the /app/consent SectionPlaceholder (rail destination already exists). PORTS design-reference screen 13 ledger table (Customer · purpose+version · captured · status + filter chips); the per-proof history timeline + made-under provenance + withdrawal action + cascade preview are DERIVED additions (P-XII — screen 13 has the ledger only). THE SHARED READ: cascade-preview N = clips a withdrawal withholds = the proof's clips = the made-under provenance list → ONE read getProofConsentClips (N = list length). Additive reads: getConsentLedger (reuses latestConsentState/Version/EffectiveAt), getConsentHistory (all retained versions, version asc), getProofConsentClips. New write recordConsentWithdrawal: re-check granted (reuse getGrantedConsentId), insert existing-shape consent row state='revoked' version=max+1 revokedAt=now (single-attempt, D4; unique (proofId,version) guards; prior versions RETAINED). recordWithdrawal action → revalidate cluster (consent/dashboard/library/showcase/proof) so the FREE CASCADE shows. Honest semantics: RECORDS the customer's withdrawal (not brand-revoke); NO re-grant (re-consent is T7); stored enum stays 'revoked', UI says "withdrawn"/"Record withdrawal". NO schema change, NO new dep, NO new gate, NO new route. PLAN done — no ratification. Byte-stable: effectiveConsentState/latestConsentState/getGrantedConsentId + EVERY consuming surface (inbox/ProofCard, Library, dashboard, export, warmth, showcase, clip detail, proof detail, generateClip, generateBatch) + nav rail — the cascade is free via existing read-time gates.)
<!-- SPECKIT END -->
