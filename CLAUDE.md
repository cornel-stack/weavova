...
`specs/T5-brand-kit/plan.md` (active slice: T5-BrandKit — Brand kit: store the brand's visual identity (logo, colours, fonts) so output looks like THEIR brand. Distinct from B2's footage store (this is visual identity). Q1:A single kit per workspace (/app/brand, no list/[id]); Q2:A curated font selection (no upload); Q3:A one brand colour + auto-contrast; single logo v1 (light/dark deferred). Replaces the /app/brand SectionPlaceholder (rail destination exists). PARTIAL HONEST PORT of screen-12 editor — logo+colour+fonts ONLY; HIDDEN (not faked/dead, per port-completeness rule + P-XII): the "live preview · reskins live" (=forbidden styled-clip preview → T8 seam), B-roll cutaways (=B2 footage /app/footage), music beds + per-format caption styles (=T8). Logo upload REUSES B2's r2.ts presigned-PUT + aws4fetch (NO new dep, same R2 env/CORS; new image allowlist mirroring B2's brand-asset.ts); real logo→<img>, absent→honest "no logo yet", never broken. ONE brandColor stored + contrast DERIVED at read time (contrastOn(hex), not stored). Curated fonts from renderable Pressroom set. Honest identity preview (logo/swatch+derived contrast/font specimens all real) + labeled T8 seam for clip application (NO faked preview). Owned brand data — NO consent (like B2). SCHEMA: additive brand_kit table (migration 0003: id, workspaceId, name?, logoAssetUrl?, brandColor, fonts jsonb; NO unique on workspaceId → naturally multi-row so multi-kit later is additive UI no migration; v1 upsert the single row). New src/lib/brand-kit.ts (BrandKitView, FONT_OPTIONS, contrastOn, ALLOWED_LOGO_TYPES, validation). Reads getBrandKit + upsertBrandKit; actions presignBrandKitLogoUpload + saveBrandKit. PLAN done — no ratification. Byte-stable: ProofCard, proof/clip/showcase/consent reads, generateClip, generateBatch, nav rail; r2.ts presignPut/assetUrlForKey reused unchanged; build green without R2 env.)
<!-- SPECKIT END -->
