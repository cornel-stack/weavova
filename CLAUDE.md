...
`specs/T7.4a-consent-media-remediation/plan.md` (active slice: T7.4a — Consent-Media Remediation. Closes the P-VII gap the T7.4 trace found: captured customer media was persisted as a PUBLIC unsigned permanent URL, consent withdrawal never reached the R2 file, and the worker GET was malformed (received a URL where a key is expected → normalize broken on real data). THE CORE: two object classes, two access models. CONSENT-BEARING customer media (captured + normalized proof media) = PRIVATE; BRAND assets (logos/brand-kit/footage via assetUrlForKey) = PUBLIC, UNCHANGED. Q1 RESOLVED → A (separate buckets): keep the existing PUBLIC brand-assets bucket (R2_BUCKET / R2_PUBLIC_BASE_URL / media.weavova.com) + add a NEW PRIVATE customer-media bucket (R2_CAPTURES_BUCKET, NO public domain, presigned-GET reads only) → customer media is private BY CONSTRUCTION (assetUrlForKey physically cannot address it). FIXES: (1) capture stores the KEY not assetUrlForKey(...) — src/app/c/[token]/actions.ts persists proof.mediaUrl=key via presignCaptureUpload; /c PAGE untouched (action only). (2) worker GET fix — worker/src/r2.ts R2_BUCKET→R2_CAPTURES_BUCKET, getObject(key) now correct, normalize ENCODE logic unchanged, T7.4 failure contract preserved. (3) withdrawal HARD-DELETES the file (P-VII) — add deleteCaptureObject (none existed); recordConsentWithdrawal (queries.ts, DATA LAYER so no UI bypass) writes the revoked version FIRST then hard-deletes captured+normalized objects; idempotent (404=success, text/no-media=no-op); consent RECORD retained for audit, MEDIA destroyed. (4) MEDIATED READ pattern established not built — presignCaptureRead(key) signed+expiring, the ONLY sanctioned customer-media read (T8 playback's path); NO playback built, the T7.2 non-playing seam BYTE-STABLE. (5) brand assets UNCHANGED (assetUrlForKey + public path regression-guarded). r2.ts: getPublicConfig (existing) + getCapturesConfig (new); class-named helpers (presignPut/assetUrlForKey=PUBLIC vs presignCaptureUpload/presignCaptureRead/deleteCaptureObject=CAPTURES) so cross-routing is impossible. NO MIGRATION (proof.mediaUrl/normalized_media_url already exist, just hold keys now). NO BACKFILL (verified live: 15 proofs, 0 with media, 0 public-URL values — all seeded fixtures; re-verify count at ship). NO NEW DEP (presign/delete reuse aws4fetch; app stays 11). CORES FROZEN (P-V): capture_request token model, T7.5 resolver/verification.ts/qualifyingBasisExpr, /c/[token] PAGE UI, consent schema, normalize ENCODE logic, assetUrlForKey/brand path — ENUMERATED touch points only (r2.ts two-bucket+captures helpers, capture action media-persist line, recordConsentWithdrawal delete cascade, worker r2 bucket-routing+delete, env/runbook); STOP if a core needs real change. PROVISIONING (Cornel-owned, build green without): create the 2nd PRIVATE R2 bucket (no public domain) + set R2_CAPTURES_BUCKET on app/Vercel/worker. TWO FLAGGED DECISIONS for /speckit.tasks: D6 normalize-after-withdrawal orphan race (recommend A: worker re-checks effective consent before writing normalized — small worker-logic touch; fallback B: delete-original makes in-flight getObject 404 → clean fail + future sweep); D7 same R2 token for both buckets (recommend, simplest) vs separate token (least-privilege hardening). Constitution: P-VII CENTRAL (revocation cascades to the FILE not just app surfaces), P-III (no new dep; 2nd bucket=config not a library), P-V (cores frozen, enumerated), P-XIII (presign-read=real plumbing, no playback control pre-T8, non-playing seam honest), P-XIV (media_status honest, failure contract preserved), P-XV/XVI N/A (non-render). PLAN + research(D1–D8, D6/D7 flagged)/data-model(no migration)/contracts(r2-bucket-routing+withdrawal-cascade+presign-read)/quickstart done. No open [NEEDS CLARIFICATION] (Q1 resolved A; D6/D7 flagged with recommended defaults). PRIOR SHIPPED: T7.4 increments 1+2 (committed) — app-side ingestion spine (generic webhook, medium-basis bridge, migration 0010) + Railway normalize worker (separate deployable, shared schema via repo-root context, request.created+media.captured Inngest functions, two-layer idempotency, provisioning runbook); app stays 11 deps, cores frozen.)
<!-- SPECKIT END -->
