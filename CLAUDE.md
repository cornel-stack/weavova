...
`specs/T6.2-onboarding-wizard/plan.md` (active slice: T6.2 — Onboarding Wizard, the workspace-configuration flow that runs after T6.1 creates a workspace. STEP 0 design-sync PASSED: all 5 designs present in design-reference/Weavova/Onboarding/ as paired HTML+PNG (1 business type _onboard_role, 2 connect source _onboard_source, 3 brand quickstart _onboard_brand, 4 first format _onboard_format, 5 dashboard spotlight tour) — ported faithfully, verbatim copy, light+dark, Pressroom. THE FLOW: 4-step wizard (Business→Source→Brand→Format) + a 5-step dashboard spotlight tour. READINESS MAP (honest where deferred): Step1 business_type FULLY REAL (new additive column); Step2 source PARTIAL — the generic WEBHOOK path is REAL (reuse getOrCreateWebhookEndpoint + T7.4 per-workspace secret, NO duplication), native connectors Shopify/Stripe/Instagram are honest "coming" (reuse T7.3 request-builder pattern), NO OAuth built, Sources track later fills OAuth BEHIND the same buttons (clean seam); Step3 brand LARGELY REAL — reuse existing brand actions presignBrandKitLogoUpload (PUBLIC bucket via assetUrlForKey, NOT private captures) + saveBrandKit/upsertBrandKit, NO new brand model; Step4 first_format PREFERENCE-ONLY (new additive column) — design 4 has ZERO preview/render text so the format tiles are DECORATIVE concept-art ported STATIC, NO clip, NO fabricated preview (P-XIV); Step5 tour = non-blocking one-shot client overlay on the REAL honest-EMPTY dashboard (zeroed masthead, no fabricated Lumen sample data). THE ROUTING GATE (T6.1 seam): lives in the Layer-2 LAYOUTS not middleware (middleware is deliberately DB-free cookie-presence) — FORWARD gate in src/app/app/layout.tsx (onboarded_at IS NULL → redirect /onboard/role), INVERSE gate in NEW src/app/onboard/layout.tsx (onboarded_at set → redirect /app, blocks re-entry); getCurrentWorkspace ALREADY returns onboarded_at (T6.1 Option A) so the gate is a FREE read, session.ts UNTOUCHED; middleware matcher extended to include /onboard/:path*. FINISH/SKIP both call markWorkspaceOnboarded (onboarded_at=now): Finish→/app?tour=1 (one-shot tour trigger, no persisted flag), Skip→/app (global skip, no nag, no tour); partial-safe (each step writes independently, untouched steps stay NULL — no fabricated config). SEEDED Lumen owner (onboarded_at set by T6.1 seed) NEVER sees the wizard (forward-gate no-op). MIGRATION additive only (drizzle/0012): workspace.business_type + workspace.first_format nullable text, code-side allowlist (source.kind precedent — NOT enum); onboarded_at already exists (T6.1); brand_kit/webhook_endpoint UNCHANGED. NEW data-layer writes (queries.ts): setWorkspaceBusinessType/setWorkspaceFirstFormat (allowlist-guarded) + markWorkspaceOnboarded. CORES FROZEN (P-V): brand-kit/webhook models + their actions, assetUrlForKey public/private split (T7.4a), T6.1 bootstrap, consent/verification — reuse not rebuild; STOP if a core needs real change. NO NEW DEP, NO OAuth, NO render. Constitution: P-V (5 screens ported, binding refs; reuse T6.1/T7.4/brand/dashboard), P-XIII (step2 native + step4 render honest coming/preference, step2 webhook LIVE, no dead controls), P-XIV (no fabricated previews/counts/sample data; owned data only), P-III (no new dep/provider), P-XV/XVI N/A (non-render). PLAN + research(D1 gate=layouts, D2 step2 seam, D3 step4 decorative-not-render, D4 tour query-param one-shot, D5 resume=step1-prefill, D6 text+allowlist)/data-model(2 additive columns + allowlists, reused models)/contracts(routing-gate + step-writes + coming-states)/quickstart(A–G + resume) done. No open [NEEDS CLARIFICATION] (5 settled + D3 resolved by design inspection). PRIOR SHIPPED: T6.1 signup→workspace bootstrap (committed+pushed to main, migration 0011 onboarded_at applied); T7.4a + T7.4; T6 auth; T0–T3 + Showcase wall to prod.)
<!-- SPECKIT END -->
