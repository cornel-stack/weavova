...
`specs/T7.2b-capture-breadth/plan.md` (active slice: T7.2b — Capture Breadth: photo + audio capture paths + the polished camera-blocked (09) and expired-link (10) surfaces. The deferred remainder of T7.2 — the text+video capture SPINE is live+FROZEN; this widens it to all four proof types + builds the two edge surfaces. STEP 0 design-sync PASSED for 08/09/10 (paired HTML+PNG in design-reference/Weavova/Capture/); AUDIO has NO dedicated screen → REUSE the 02/03 recording/review pattern with an audio treatment (mic record, timer, NO video preview, audio playback on review) — a documented adaptation, NOT an invented design (P-V/P-XII). GROUNDING SHRANK THE SLICE: proof_type enum ALREADY has photo+audio (NO migration); worker photo-resize built + audio-SKIP built in media-captured.ts (NO worker change); media.captured emits generically from writeCapturedProof; withdrawal cascade deletes by KEY (photo/audio covered by T7.4a). So this is CLIENT-FLOW work + an additive upload allowlist + the 09/10 ports + ONE honest-copy fix. PHOTO: OS camera/file picker → review (08) → SAME consent (04, WITH face-display — a photo may show a face) → SAME atomic send; proofType=photo, private-bucket KEY, worker resizes → normalized. AUDIO: MediaRecorder audio → review w/ playback → consent (WITHOUT face-display) → send; proofType=audio, private KEY; worker SKIPS audio → media_status stays 'captured' = COMPLETE not pending. AUDIO-HONESTY (verified): ProofView/ProofDetailView do NOT carry media_status (only proof_type+mediaUrl) → NO merchant surface reads media_status → audio at 'captured' renders IDENTICALLY to a normalized proof (non-playing seam); nothing treats it as stuck. THE ONE DRIFT: proof-detail-media.tsx hard-codes "video stored" badge → generalize to proof_type ("photo/audio/video stored · playback coming", P-XIV honest-copy fix). SCREEN 09 (camera-blocked): port polished over the existing upload fallback, verbatim ("No camera? No problem…" + Upload from gallery / Write it instead), same behavior. SCREEN 10 (expired): port over block.tsx; workspace-name personalization for expired/USED (getCaptureRequestByToken ALREADY selects workspace.name — additive: return it on used/expired; page.tsx passes to CaptureBlock), GENERIC for not-found (no row→no name→no leak); "Ask {Workspace} for a new link" = HONEST GUIDANCE TEXT, NOT a fake/dead button (no customer→merchant channel, P-XIII). TOUCH POINTS (enumerated, additive): src/lib/capture.ts (all 4 paths wired + image/audio allowlists + CaptureResolution.workspaceName), actions.ts (isAllowedVideoType→isAllowedCaptureType video∪image∪audio; submitCapture path union + photo/audio branches; presignCaptureUpload still PRIVATE bucket+KEY), capture-flow.tsx (photo flow + audio flow + screen-09 fallback + drop coming view + showFace→photo), block.tsx (screen-10 port), page.tsx (pass workspaceName), queries.ts getCaptureRequestByToken (return workspaceName on used/expired), proof-detail-media.tsx (badge fix). FROZEN (P-V, no change): atomic send/consume core, token model, consent write, video/text paths, T7.4a storage/routing, the worker, media.captured emit, ProofView/queries (zero read-edit), proof-card.tsx (already generic). NO MIGRATION, NO NEW DEP (MediaRecorder audio + file-input image are browser APIs; 11 deps). Constitution: P-V (08/09/10 ported verbatim + audio-pattern adaptation, spine frozen, layout-faithful first pass), P-VII (same scoped consent 04 + private T7.4a KEY storage + withdrawal-reaches-file; showFace on photo), P-XIII (photo/audio coming→live, 09/10 real, audio honest-complete not faked, "Ask" = guidance not dead button), P-XIV (real media, honest status, badge mislabel fixed), P-XV/XVI N/A (worker normalize=media prep not composition). PLAN + research(D1 audio=02/03 pattern, D2 allowlist+HEIC, D3 audio-status honesty=no media_status consumer, D4 screen-10 name, D5 "Ask"=guidance, D6 showFace photo-yes audio-no, D7 badge fix)/data-model(no schema change, +CaptureResolution.workspaceName, +image/audio allowlists)/contracts(capture-paths + edge-surfaces)/quickstart(A–D + headless) done. No open [NEEDS CLARIFICATION] (audio-pattern confirmed; all decisions grounded). PRIOR SHIPPED: T6.2 onboarding wizard (committed+pushed, migration 0012); T6.1 (migration 0011); T7.4a + T7.4; T6 auth; T0–T3 + Showcase wall to prod.)
<!-- SPECKIT END -->
