...
`specs/T7.4-ingest-normalize-webhook/plan.md` (active slice: T7.4 — Ingest + normalize worker + generic inbound webhook. FIRST two-deployable / cloud-worker slice — makes the capture funnel AUTOMATIC + stands up the Railway media host (shared with T8's renderer later). THREE pieces: (1) generic inbound webhook (Vercel route src/app/api/ingest, NO OAuth) — per-workspace SHARED-SECRET auth, mints capture_request via the EXISTING T7.2 primitive (createCaptureRequest, token/72h/single-use UNCHANGED), fetch-emits Inngest request.created; the UNIVERSAL DOOR (Zapier/Make/n8n bridge anything; native Shopify/Stripe/Calendly OAuth DEFERRED to Sources track). (2) normalize worker = FIRST Railway service (worker/ dir, separate deployable) — Inngest serve+functions + ffmpeg; media.captured → normalize (video ≤1080p H.264/AAC + rotation bake; photo resize; audio deferred — no audio capture yet); idempotent keyed on media_status; T8 renderer = planned SECOND service same host. (3) webhook → MEDIUM verification_basis: webhook sets capture_request.transaction_verified_at (additive) → writeCapturedProof writes medium/source=webhook basis → T7.5 stamp lights up via the FORWARD CONTRACT, RESOLVER UNCHANGED. TOPOLOGY (D1): app emits Inngest via fetch to Event API (NO SDK → app STAYS 11 deps); Inngest SDK + ffmpeg = WORKER-side deps (separate). EMIT POINTS: webhook→request.created; writeCapturedProof→media.captured (NOT the frozen /c route, D7). IDEMPOTENCY both layers: webhook_event ledger (event_id/derived key, D4); normalize keyed on media_status (act on captured/normalizing, skip normalized, deterministic output key). RE-SYNC (D8, Bristle cron-stall lesson): worker auto-syncs functions on EVERY Railway redeploy + /health check — MANDATORY (functions silently stop registering after redeploy). Migration 0010 ADDITIVE: media_status enum + proof.normalized_media_url/media_status + capture_request.transaction_verified_at + webhook_endpoint(per-ws secret, plain high-entropy, REAL authenticating per P-XIII)/webhook_event tables + "webhook" in SOURCE_KINDS. Secret surface = minimal honest spot (dev/styleguide+trigger area until T9 Settings; real secret, not decorative). CORES FROZEN (P-V): /c/[token], token model, T7.5 resolver/verification.ts/qualifyingBasisExpr, consent — ENUMERATED touch points (writeCapturedProof basis branch+emit, createCaptureRequest +transactionVerifiedAt opt, schema/queries additive); STOP if a core needs real change. PROVISIONING (Cornel-owned, T6 pattern, build green without): app env INNGEST_EVENT_KEY/EVENT_API_URL; worker env INNGEST_SIGNING_KEY+R2_*+DATABASE_URL+AUTH_RESEND_KEY/EMAIL_FROM; Railway worker/Dockerfile (ffmpeg, repo-root build context). Constitution: P-III (Inngest in stack now wired; Railway host; app 11 unchanged, ffmpeg worker-side), P-V (cores frozen), P-XIII (real authenticating secret, native "coming"), P-XIV (honest medium basis only on real evidence; generic-live/native-coming truthful), P-XV/XVI N/A (normalize=media prep not composition). PLAN + research(D1–D10)/data-model/contracts(webhook-inbound+inngest-events+worker-host)/quickstart done. ONE flagged decision: worker schema-sharing (D9 — shared schema.ts import + next-auth type devDep vs worker-local schema) — confirm at /speckit.tasks. No open [NEEDS CLARIFICATION]. PRIOR SHIPPED: T7.5 — Verification basis, the TRANSACTION leg of "Verified real" so the stamp is EARNED not asserted. NON-RENDER, model + stamp-logic slice, small UI footprint. CORE: a single sanctioned resolver proofIsVerified/verificationState (src/lib/verification.ts, NEW) = consent-AND-basis; proof.verified stops being a static boolean read — it's DERIVED. CHOKEPOINT (D1, P-XIV): NO projection selects proof.verified anymore (removed from every row type); projections select effective consentState + a NEW internal hasQualifyingBasis (qualifyingBasisExpr EXISTS, beside effectiveConsentState in queries.ts); view mappers compute verified via the resolver; proof.verified column kept but marked write-frozen/internal (additive migration — NOT dropped; drop is a flagged future option B, confirm at /speckit.tasks). BAR (Q1=A, FR-019): strength IN (strong,medium) AND transaction_verified_at NOT NULL; weak/manual (a merchant ASSERTION, not a CONFIRMATION) NEVER earns it. THREE states: verified / consent_only / unverified_no_consent. GRADED MODEL (D2): migration 0009 ADDITIVE — basis_strength enum(strong/medium/weak) + basis_source enum(native/webhook/manual) + verification_basis.transaction_ref + transaction_verified_at made real (already-nullable, now written) + verification_basis.request_id DROP NOT NULL (native/webhook/seed bases have NO capture_request). FORWARD CONTRACT (D5): deferred T7.4 webhook / native connectors just WRITE a typed-strength basis — resolver UNCHANGED, no rework. LIVE NOW (Q2=A, D6): nothing live earns strong/medium (Sources deferred) — writeCapturedProof attaches source=manual/strength=weak/transaction_ref=assertion/verified_at=null → link/manual proof shows the honest in-between; /c/[token] route UNTOUCHED (FR-014, helper-only edit). IN-BETWEEN (Q3=A, P-XII/P-IV): design has NO unverified badge (binary model) → QUIET derived state: stamp ABSENCE on cards, a non-persimmon "Consent recorded · transaction unconfirmed" label on proof DETAIL only (the ONE new UI; proof-detail-meta.tsx). BACKFILL (D7, idempotent delete+reinsert): 5 verified fixtures (Darnell/Maria/Aisha/Hannah/Greta) get strong native bases that legitimately earn the stamp; granted-consent unverified → weak; awaiting/revoked → none. P-VII (SC-004): Leo M. (revoked) NOT verified even with a strong basis — resolver short-circuits on consent. SWAP (D8, P-V): 11 read sites + 5 view types (ProofView/ProofDetailView+verificationState/LibraryClipView/ClipDetailView/PostTextPackage) route through resolver, BYTE-STABLE except detail label. Constitution: P-XIV/FR-019 (the bar, no over-claim, structural chokepoint), P-V (byte-stable), P-VII (consent necessary), P-XIII (honest in-between), P-IV (persimmon scarce — absence on cards), P-XV/XVI N/A. PLAN + research(D1–D8)/data-model/contracts(proof-verified-resolver + consumer-swap)/quickstart done. ONE flagged decision: drop vs retire-in-place proof.verified (D1) — confirm at /speckit.tasks. No open [NEEDS CLARIFICATION].)
<!-- SPECKIT END -->
