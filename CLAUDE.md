...
`specs/T7.2-capture-spine/plan.md` (active slice: T7.2 — Capture spine + request primitive; the heart of T7. The PUBLIC, UNAUTHENTICATED `/c/[token]` flow where a REAL customer submits proof + grants real T7.1 scoped consent — first real consented proof enters the system. UI SLICE: faithful mobile-first PORT of design-reference/Weavova/Capture/ screens 01–07 (STEP 0 confirmed the folder exists — 10 screens; verbatim copy lifted), Pressroom tokens light+dark. SCOPE = SPINE: video path (01→02→03→04→05→06 via MediaRecorder+upload fallback) + text path (01→07→04→05→06); photo/audio/camera-blocked(09)/polished-expired(10) = T7.2b honest coming-states (P-XIII). 5 RESOLVED ARCH DECISIONS: (1) PROOF-SOURCE crux — proof.sourceId NOT NULL → add 'link' to code-side SOURCE_KINDS (no migration), request carries sourceId, captured proof shows source.label 'Capture link'; reads select source.LABEL not kind → ZERO owner-surface edits (getProofs/getProof/getDashboardSummary verified). (2) UNAUTH route — /c/[token] already OUTSIDE middleware matcher ['/app/:path*']; inherits root layout only (no chrome); workspace+brand resolved FROM TOKEN (token→request→workspace→getBrandKit), actions are TOKEN-SCOPED (never getCurrentWorkspace). (3) ATOMIC SEND on neon-http (NO interactive txn) — consume-first conditional `UPDATE capture_request SET status=used WHERE token=$1 AND status=open AND expires_at>now() RETURNING` (atomic single-use guard) THEN db.batch([proof, consent, verification_basis]) with client-gen UUIDs (one txn, no partial proof; post-consume failure burns token honestly). (4) MEDIA — reuse B2 presigned-PUT (browser PUTs bytes direct to R2, never Vercel); new additive captureMediaKey() in r2.ts. (5) VERIFICATION BASIS = separate additive table (consent_captured_at REAL + transaction_verified_at NULL stub→T7.5); keeps proof shape fixture-identical; proof.verified stays false, NO stamp. CONSENT via T7.1: least-privilege useScope=['organic'], display via resolveDisplay (sole sanctioned write path, more-private-only, server-enforced), screen-04 wording + read-full-terms + takedown copy. Migration 0006 additive: enum capture_request_status(open|used|expired) + capture_request table + verification_basis table. Token: per-request single-use 72h expiry (CAPTURE_TOKEN_TTL_HOURS; expires_at>now() authoritative). Request creation = seeded + dev-only link display (copyable URL); webhook+Resend = T7.3. INTEGRATION (FR-015, zero-edit): inbox/dashboard/proof-detail/consent-ledger/studio gate render capture proof through existing reads UNCHANGED — if any needs an edit, STOP (P-V violation). No new dep (we're at 11) UNLESS C1→B. Constitution: P-V (port + byte-stable consumers), P-II, P-VII (real scoped consent), P-VIII (record→review, no editor), P-XIII (coming-states honest, no faked stamp), P-XIV, P-XV/XVI N/A. PLAN + research/data-model/contracts(capture-actions+capture-ui)/quickstart done; ONE open clarification C1 (QR: link-only [rec] vs add QR dep) — answer before /speckit.tasks.)
<!-- SPECKIT END -->
