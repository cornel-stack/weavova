...
`specs/T6-auth-workspaces/plan.md` (active slice: T6 — Real authentication + workspaces. Retires the stub session (`src/lib/session.ts`) for real Auth.js v5 (NextAuth v5) + Drizzle adapter; TWO providers: email magic-link via Resend + Google OAuth; same verified email → ONE user (link by verified email, Google `allowDangerousEmailAccountLinking` — safe ONLY because both providers present verified emails). SCHEMA additive migration 0004: adapter tables user/account/session/verification_token + app `membership` (role enum owner|member from day one — FR-018; unique (userId,workspaceId)); `workspace` UNCHANGED. The seam swap is the P-V core: `getSession()`/`getCurrentWorkspace()` become session-backed, SHAPES PRESERVED, so all 11 FR-021 pages + 8 Server Actions stay byte-stable (they already route through the seam; every query already takes workspaceId). Security TWO layers (FR-008+FR-017): `middleware.ts` gate on /app/* AND workspace-scoped reads (requireWorkspace() helper). Routing: unauth / → /login, auth / → /app (FR-020). Auth UI: /login (both providers), /verify (check-email), sign-out in UserMenu. Invites OUT (fast-follow) → honest "Invite teammates — coming soon" affordance (FR-019, P-XIII). DB-session strategy; workspaceless users auto-provisioned a personal workspace+owner membership. Fixture reconciliation: content rows ALREADY workspace-scoped → NO ownership backfill; seed/idempotent-backfill adds a demo user + owner membership over Lumen. Couplings resolved: styleguide/data → dev-only (reads getDefaultWorkspace directly, 404 in prod); error.tsx UNCHANGED (auth/no-workspace paths redirect, never throw). NEW deps flagged (+2 → 11): next-auth@beta, @auth/drizzle-adapter (built-in Resend+Google providers, NO resend SDK). Env Cornel provisions: AUTH_SECRET, AUTH_GOOGLE_ID/SECRET, AUTH_RESEND_KEY, AUTH_EMAIL_FROM, AUTH_TRUST_HOST (+ SEED_OWNER_EMAIL). Build green without AUTH_* (lazy). P-VII consent UNCHANGED. PLAN done; TWO open clarifications C1 (demo-owner email) + C2 (invites affordance placement) — answer before /speckit.tasks.)
<!-- SPECKIT END -->
